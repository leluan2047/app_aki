...
class PlgExpandProductColumnsCsvImportController extends \Eccube\Controller\Admin\Product\CsvImportController
{
            ...
            if ($form->isValid()) {
                ...
                if (!empty($formFile)) {
                    ...
                    foreach ($data as $row) {
                        ...
                        //kozoの追加 - Add maker and url
                        $this->createProductMaker($row, $Product, $app, $data);    
                        //End - kozoの追加 - Add maker and url
     ...
     * @param CsvImportService $data
     */
    protected function createProductMaker($row, Product $Product, $app, $data)
    {
        if ($row['メーカー'] == '') {
            // メーカーの削除    
            $this->deleteProductMaker($Product, $app);
            return;
        }
        
        // メーカーの登録
        $maker = $app['eccube.plugin.maker.repository.maker']->findBy(array('name' => $row['メーカー']));
        if ($maker) {
            ...
            $this->em->persist($ProductMaker);
        } else {
            $this->addErrors(($data->key() + 1) . '行目のメーカーが存在しません。');
            return;
        }
    }
    
    protected function deleteProductMaker(Product $Product, $app){
        $ProductMaker = $app['eccube.plugin.maker.repository.product_maker']->find($Product); 
        if (!empty($ProductMaker) && $ProductMaker->getId()) {
            $this->em->remove($ProductMaker);
        }
    }
