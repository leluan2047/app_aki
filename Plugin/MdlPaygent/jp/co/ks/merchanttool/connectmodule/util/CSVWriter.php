...
class CSVWriter {
    ...
    {
    	$this->app = $app;
    	$this->newLine = $this->app['config']['MdlPaygent']['const']['CSVWriter__WINDOWS_NEWLINE'];
    }
     ...
     */
    function CSVWriter($filePath, $encoding = $this->app['config']['MdlPaygent']['const']['CSVWriter__ENCODING_MS932'],
    		$envelop = $this->app['config']['MdlPaygent']['const']['CSVTokenizer__DEF_ITEM_ENVELOPE']) {
    ...
    function writeOneLine($line) {
		...
		if (is_string($line)) {
	        ...
	        foreach($line as $i => $data) {
	            ...
	            if ($this->envelop != $this->app['config']['MdlPaygent']['const']['CSVTokenizer__NO_ITEM_ENVELOPE']) {
    ...
    function cnvKnmString($str) {
        ...
        for ($i = 0; $i < strlen($str); $i++) {
            ...
            if ($str{$i} == $this->app['config']['MdlPaygent']['const']['CSVTokenizer__DEF_SEPARATOR']) {
                $flg = true;
            }
            $buf .= $str{$i};
        }
