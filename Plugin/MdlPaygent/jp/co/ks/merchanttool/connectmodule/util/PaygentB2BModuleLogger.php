	...
	function debug($className, $message) {
		if(is_null($this->filename) == false && $this->filename != "") {
			if(! $handle = fopen( $this->filename, 'a')) {
				// ファイルが開けない
				trigger_error($this->app['config']['MdlPaygent']['const']['PaygentB2BModuleException__OTHER_ERROR']. ":File doesn't open.(".$this->filename.").", E_USER_WARNING);
				return;
			}
			if(! fwrite($handle, $this->outputMsg($message, $className))) {
				// ファイルに書き込めない
				trigger_error($this->app['config']['MdlPaygent']['const']['PaygentB2BModuleException__OTHER_ERROR']. ":It is not possible to write it in the file(".$this->filename.").", E_USER_WARNING);
				return;
			}
			fclose($handle);
		}
