...
class ReferenceResponseDataImpl extends ResponseData {
	...
	function parse($body) {
		...
		$csvTknzr = new CSVTokenizer($this->app, $this->app['config']['MdlPaygent']['const']['CSVTokenizer__DEF_SEPARATOR'],
			$this->app['config']['MdlPaygent']['const']['CSVTokenizer__DEF_ITEM_ENVELOPE']);
		...
		$lines = explode($this->app['config']['MdlPaygent']['const']['ReferenceResponseDataImpl__LINE_SEPARATOR'], $body);
		foreach($lines as $i => $line) {
			$lineItem = $csvTknzr->parseCSVData($line);

			if (0 < count($lineItem)) {
				if ($lineItem[$this->app['config']['MdlPaygent']['const']['ReferenceResponseDataImpl__LINE_RECORD_DIVISION']]
						== $this->app['config']['MdlPaygent']['const']['ReferenceResponseDataImpl__LINENO_HEADER']) {
					// ヘッダー部の行の場合
					if ($this->app['config']['MdlPaygent']['const']['ReferenceResponseDataImpl__LINE_HEADER_RESULT'] < count($lineItem)) {
						// 処理結果を設定
						$this->resultStatus = $lineItem[$this->app['config']['MdlPaygent']['const']['ReferenceResponseDataImpl__LINE_HEADER_RESULT']];
					}
					if ($this->app['config']['MdlPaygent']['const']['ReferenceResponseDataImpl__LINE_HEADER_RESPONSE_CODE'] < count($lineItem)) {
						// レスポンスコードを設定
						$this->responseCode = $lineItem[$this->app['config']['MdlPaygent']['const']['ReferenceResponseDataImpl__LINE_HEADER_RESPONSE_CODE']];
					}
					if ($this->app['config']['MdlPaygent']['const']['ReferenceResponseDataImpl__LINE_HEADER_RESPONSE_DETAIL'] < count($lineItem)) {
						// レスポンス詳細を設定
						$this->responseDetail = $lineItem[$this->app['config']['MdlPaygent']['const']['ReferenceResponseDataImpl__LINE_HEADER_RESPONSE_DETAIL']];
					}
				} else if ($lineItem[$this->app['config']['MdlPaygent']['const']['ReferenceResponseDataImpl__LINE_RECORD_DIVISION']]
						== $this->app['config']['MdlPaygent']['const']['ReferenceResponseDataImpl__LINENO_DATA_HEADER']) {
					// データヘッダー部の行の場合
					$this->dataHeader = array();
					...
					for ($i = 1; $i < count($lineItem); $i++) {
						...
						== $this->app['config']['MdlPaygent']['const']['ReferenceResponseDataImpl__LINENO_DATA']) {
					// データ部の行の場合
					...
					if (count($this->dataHeader) == (count($lineItem) - 1)) {
						...
						$sb = $this->app['config']['MdlPaygent']['const']['PaygentB2BModuleException__OTHER_ERROR'] . ": ";
						$sb .= "Not Mutch DataHeaderCount=";
						$sb .= "" . count($this->dataHeader);
						$sb .= " DataItemCount:";
						$sb .= "" . (count($lineItem) - 1);
						trigger_error($sb, E_USER_WARNING);
						return $this->app['config']['MdlPaygent']['const']['PaygentB2BModuleException__OTHER_ERROR'];
					}
					...
					if (0 < count($map)) {
						...
						== $this->app['config']['MdlPaygent']['const']['ReferenceResponseDataImpl__LINENO_TRAILER']) {
					// トレーラー部の行の場合
					if ($this->app['config']['MdlPaygent']['const']['ReferenceResponseDataImpl__LINE_TRAILER_DATA_COUNT'] < count($lineItem)) {
						// データサイズ
					}
				}
			}
	...
	function getDataCount($body) {
		...
		foreach($lines as $i => $line) {
			...
			if (0 < count($lineItem)) {
				if ($lineItem[$this->app['config']['MdlPaygent']['const']['ReferenceResponseDataImpl__LINE_RECORD_DIVISION']]
					...
					if ($this->app['config']['MdlPaygent']['const']['ReferenceResponseDataImpl__LINE_TRAILER_DATA_COUNT'] < count($lineItem)) {
						// データ件数を取得 whileから抜ける
						if (StringUtil::isNumeric($lineItem[$this->app['config']['MdlPaygent']['const']['ReferenceResponseDataImpl__LINE_TRAILER_DATA_COUNT']])) {
							$strCnt = $lineItem[$this->app['config']['MdlPaygent']['const']['ReferenceResponseDataImpl__LINE_TRAILER_DATA_COUNT']];
						}
						break;
					}
				}
		...
		if ($strCnt != null && StringUtil::isNumeric($strCnt)) {
			...
			return $this->app['config']['MdlPaygent']['const']['PaygentB2BModuleException__OTHER_ERROR'];		//エラー
		}
